...
@section('content')
    <div class="blog-show">
        <section class="blog-content shadow-sm radius-lg">
            <div class="text">
                @if ($article instanceof \App\Models\Article)
                    <div class="heading">
            ...
            <div class="share-your-articles shadow-lg radius-lg">
                <div class="title">{{ __('main.blog_share_article') }}</div>
                <div class="links grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" class="link flex items-center justify-between gap-2">
                    ...
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&amp;text={{ urlencode($article->title ?? '') }}" class="link flex items-center justify-between gap-2">
                    ...
                    <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" class="link flex items-center justify-between gap-2">
                    ...
                    <a href="https://wa.me/?text={{ urlencode(($article->title ?? '') . ' ' . url()->current()) }}" class="link flex items-center justify-between gap-2">
                    ...
                    <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&amp;text={{ urlencode($article->title ?? '') }}" class="link flex items-center justify-between gap-2">
            ...
            @if ($similarArticles && count($similarArticles) > 0)
                <div class="main-articles-section shadow-lg radius-lg" style="padding: var(--inline-padding);">
                                ...
                                <div class="article">
                                    <div class="image">
                                        @if ($similar->thumbnail)
                                            <img src="{{ asset('storage/' . $similar->thumbnail) }}" alt="{{ $similar->translations[app()->getLocale()]['title'] ?? $similar->slug }}">
                                        @endif
                                    </div>
                                    <div class="visitor">
        ...
        <aside class="side">
            <div class="reviews-image shadow-sm radius-lg">
                <div class="reviews-image shadow-lg radius-lg">
                    <img src="{{ asset('assets/images/website/dark-reviews-bg.jpg') }}" alt="{{ __('main.first_marketing_logo') }}">
                </div>
            </div>
            @if ($categories && count($categories) > 0)
                <div class="categories shadow-sm radius-lg">
                        ...
                        @foreach ($categories as $category)
                            <li>
                                <a href="{{ route('blog.category', ['id' => $category->id]) }}">
                                    <span>{{ app()->getLocale() == 'ar' ? $category->name_ar ?? $category->slug : $category->name ?? $category->slug }}</span>
                                    <strong>{{ $category->published_articles_count ?? rand(6, 64) }}</strong>
                                </a>
                            </li>
                        @endforeach
                    </ul>
