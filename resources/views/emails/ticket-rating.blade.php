...
<body dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}" lang="{{ app()->getLocale() }}">
    <div style="background: #fff; padding: 30px; border-radius: 10px; max-width: 600px; margin: auto; box-shadow: 0 2px 8px rgba(0,0,0,0.04); border-left: 4px solid #0284c7;">
        <div style="border-bottom: 1px solid #e5e7eb; padding-bottom: 10px;border-radius: 10px 10px 0 0;">
        ...
        <div style="padding: 24px; display: flex; flex-direction: column; gap: 12px;">
            ...
            <p style="margin: 0 0 8px 0; color: #555; font-size: 15px;">{{ __('main.ticket_feedback_request') }}</p>
            <a href="{{ route('tickets.support_pro_rating', ['ticketId' => $ticket->uuid, 'token' => $ticket->token]) }}"
                style="display: inline-block; background: #2563eb; color: #fff !important; font-weight: 600; border-radius: 999px; padding: 12px 32px; text-align: center; text-decoration: none; font-size: 16px; margin: 0 auto; min-width: 180px;">{{ __('main.rate_ticket_now') }}</a>
        ...
        <div style="font-size: 12px; color: #888; border-top: 1px solid #e5e7eb; padding-top: 20px; border-radius: 0 0 10px 10px; background: #fafbfc;">
