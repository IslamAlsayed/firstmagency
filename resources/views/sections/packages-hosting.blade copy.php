<section class="section packages-sections text-center relative" id="packages-hosting">
    ...
    <div class="">
        <div class="grid grid-cols-1 sm:grid-cols-2ص lg:grid-cols-4 gap-4">
            @php
            $hostingPackages = (isset($hosting_packages) && count($hosting_packages) > 0) ? $hosting_packages : (config('packages-hosting.hosting') ?? []);
            $resellerPackages = (isset($reseller_packages) && count($reseller_packages) > 0) ? $reseller_packages : (config('packages-hosting.reseller') ?? []);
            @endphp

            @if ($hostingPackages || $resellerPackages)
...
<div class="">
    ...
    <div class="filter-btn-packages flex items-center justify-between gap-4 bg-white p-2 shadow-sm border mx-auto mb-4 rounded-[9px]">
        ...
        <button data-target="reseller" class="filter-btn-package flex items-center justify-between gap-2 cursor-pointer">
            <div class="flex items-center justify-between gap-2">
                <div class="image">
                    <img src="{{ asset('assets/images/website/reseller-packages/icon.png') }}" alt="">
                </div>
                <span class="font-semibold">{{ __('main.hosting_packages_reseller') }}</span>
            </div>
            <i class="fas fa-chevron-down"></i>
...
<div class="">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        @php
        $packages = \App\Models\HostingPackage::orderBy('order')->get();
        $hostingPackages = $packages->where('category', 'hosting')->values();
        $resellerPackages = $packages->where('category', 'reseller')->values();
        $vpsPackages = $packages->where('category', 'vps')->values();
        $serverPackages = $packages->where('category', 'servers')->values();
        @endphp

        @foreach ($hostingPackages as $package)
        <div class="package-card{{ $package->is_popular ? ' card-border' : '' }}" data-category="hosting">
            <div class="relative p-3">
                ...
                <div class="mb-6">
                    <div class="image">
                        ...
                        <img src="{{ asset('storage/' . $package->image) }}" class="w-16 mx-auto mb-4"
                            alt="{{ $package->translations[app()->getLocale()]['title'] ?? 'Package' }}">
                        @else
                        <img src="{{ asset('assets/images/website/placeholder.png') }}" class="w-16 mx-auto mb-4" alt="{{ __('main.placeholder') }}">
                        @endif
                    </div>
                    <h3 class="text-xl font-semibold">{{ $package->translations[app()->getLocale()]['title'] ?? '--' }}</h3>
                </div>

                <div class="mb-8">
                    <div class="flex justify-center items-start">
                        <span class="font-semibold">$</span>
                        <span class="font-semibold main-price monthly_price hidden">{{ number_format($package->monthly_price, 2) }}</span>
                        <span class="font-semibold main-price yearly_price">{{ number_format($package->yearly_price, 2) }}</span>
                    </div>
                    <p class="text-gray-600 text-sm mt-1">{{ __('main.hosting_packages_yearly_label') }}</p>
                </div>
                ...
                <ul class="space-y-4 mb-8">
                    ...
                    <li class="feature-card flex items-center justify-between text-right group/item mb-2">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span
                                class="text-gray-700 text-sm font-medium mr-auto">{{ $feature[app()->getLocale() === 'ar' ? 'title_ar' : 'title_en'] ?? ($feature['title_en'] ?? '--') }}</span>
                        </div>
                        <span
                            class="main-label text-gray-600 cursor-pointer font-semibold text-xs border border-gray-300 rounded-full w-5 h-5 flex items-center justify-center"
                            data-label="{{ $feature[app()->getLocale() === 'ar' ? 'label_ar' : 'label_en'] ?? ($feature['label_en'] ?? '--') }}">?</span>
                    </li>
                    @endforeach
                </ul>
        ...
        @if ($hostingPackages->isEmpty() && config('packages-hosting.hosting'))
        @foreach (config('packages-hosting.hosting') as $package)
