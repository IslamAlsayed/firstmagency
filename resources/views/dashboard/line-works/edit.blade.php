...
@section('title', __('main.edit_line_work'))
@section('page-title', '⚙️ ' . __('main.edit_line_work'))

@section('content')
    <div class="shadow-lg radius-lg p-4">
        <form method="POST" action="{{ route('dashboard.line-works.update', $lineWork) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid gap-4 lg:gap-6">
                ...
                <div class="language-content" data-lang="en">
                    <div class="grid gap-4">
                        <div>
                            ...
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" id="title_en" name="title_en" required
                                value="{{ old('title_en', $lineWork->translations['en']['title'] ?? '') }}" placeholder="Enter title in English">
                            @error('title_en')
                        ...
                        <div>
                            ...
                            <textarea class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 @error('description_en') border-red-500 @enderror" id="description_en"
                                name="description_en" rows="5" placeholder="Enter description in English">{{ old('description_en', $lineWork->translations['en']['description'] ?? '') }}</textarea>
                    ...
                    <div class="grid gap-4">
                        <div>
                            ...
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" id="title_ar" name="title_ar" required
                                value="{{ old('title_ar', $lineWork->translations['ar']['title'] ?? '') }}" placeholder="أدخل العنوان بالعربية">
                            @error('title_ar')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 @error('description_ar') border-red-500 @enderror" id="description_ar"
                                name="description_ar" rows="5" placeholder="أدخل الوصف بالعربية">{{ old('description_ar', $lineWork->translations['ar']['description'] ?? '') }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="alt_text" value="{{ old('alt_text', $lineWork->alt_text) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $lineWork->order) }}"
                ...
                <!-- Update Button -->
                @include('dashboard.components.update-submit', ['models' => 'dashboard.line-works', 'model' => 'line_work'])
            </div>
        </form>
    </div>
@endsection
