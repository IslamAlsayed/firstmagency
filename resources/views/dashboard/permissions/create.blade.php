...
@section('title', __('dashboard.create_permission'))
@section('page-title', '🔑 ' . __('dashboard.create_permission'))

@section('content')
    <div class="kt-card mb-4">
        <div class="kt-card-header">
            <div class="flex items-center gap-4">
                <h3 class="kt-card-title">{{ __('dashboard.create_new_permission') }}</h3>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('dashboard.permissions.store') }}">
                ...
                <div class="grid gap-4 lg:gap-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">{{ __('dashboard.permission_title') }} <span
                            ...
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                ...
                                title="{{ __('dashboard.use_lowercase_numbers') }}">
                            @error('name')
