    ...
    <div class="kt-card mb-4">
        <div class="kt-card-header">
            <div class="flex items-center gap-4">
                <h3 class="kt-card-title">{{ __('main.edit_permission') }}: {{ $permission->name }}</h3>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('dashboard.permissions.update', $permission->id) }}">
                ...
                <div class="grid gap-4 lg:gap-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">{{ __('main.permission_name') }} <span
                            ...
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                ...
                                pattern="[a-z0-9\-]+" title="{{ __('main.use_lowercase_numbers') }}">
                            @error('name')
