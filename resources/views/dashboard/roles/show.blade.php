        ...
        <div>
            <div class="background rounded-lg shadow p-4">
                ...
                <div class="flex gap-2 mt-6">
                    <a href="{{ route('dashboard.roles.edit', $role->id) }}" class="w-fit px-4 py-2 kt-btn kt-btn-outline-primary text-white rounded-lg text-center text-sm font-medium transition"
                        style="color: var(--text_color); background-color: var(--button_color);" toggle-button>
                        <i class="fas fa-edit mr-2"></i> {{ __('main.edit') }}
                    </a>
                    <a href="{{ route('dashboard.roles.index') }}" class="w-fit px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 text-center text-sm font-medium transition">
                                ...
                                <div class="space-y-2">
                                    @foreach ($modulePermissions as $permission)
                                        <div class="flex items-center gap-2 p-2 bg-gray-50 rounded">
                                            ...
                                            <span class="text-sm text-gray-700">{{ __('main.' . str_replace(' ', '-', str_replace('_', '-', $permission->name))) }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
