    ...
    <div class="w-full">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="text-center p-4 bg-gray-50 rounded-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-gray-800">{{ $marketingPackages->total() }}</div>
                <small class="text-primary font-semibold">{{ __('main.total_types', ['types' => __('main.marketing_packages')]) }}</small>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-green-600">{{ $marketingPackages->where('is_active', true)->count() }}</div>
                <small class="text-primary font-semibold">{{ __('main.active') }}</small>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-red-600">{{ $marketingPackages->where('is_active', false)->count() }}</div>
                <small class="text-primary font-semibold">{{ __('main.inactive') }}</small>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-blue-600">{{ $marketingPackages->where('is_popular', true)->count() }}</div>
                <small class="text-primary font-semibold">{{ __('main.popular') }}</small>
            </div>
        </div>
