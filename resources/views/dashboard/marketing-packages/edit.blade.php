...
@section('title', __('main.edit_type', ['type' => __('main.marketing_package')]))
@section('page-title', '📦 ' . __('main.edit_type', ['type' => __('main.marketing_package')]))

@section('content')
    <div class="kt-card mb-4">
        <div class="kt-card-header flex items-center justify-between gap-4">
            <h3 class="kt-card-title">{{ __('main.edit_type', ['type' => __('main.marketing_package')]) }}</h3>
        ...
        <div class="kt-card-body p-4">
            <form method="POST" action="{{ route('dashboard.marketing-packages.update', $marketingPackage->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid gap-4 lg:gap-6">
                        ...
                        <div class="grid gap-4">
                            <div>
                                ...
                                <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" id="title_en" name="title_en"
                                    value="{{ old('title_en', $marketingPackage->translations['en']['title']) }}" placeholder="Enter title in English">
                                @error('title_en')
                        ...
                        <div class="grid gap-4">
                            <div>
                                ...
                                <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" id="title_ar" name="title_ar"
                                    required value="{{ old('title_ar', $marketingPackage->translations['ar']['title']) }}" placeholder="أدخل العنوان بالعربية">
                                @error('title_ar')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" id="order" name="order" min="0"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" value="{{ old('order', $marketingPackage->order) }}"
                                placeholder="0">
                            @error('order')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                    ...
                    <!-- Photo Upload -->
                    @include('dashboard.components.photo', ['column' => 'image', 'model' => $marketingPackage, 'folder' => 'marketing-package'])
                    ...
                    <div class="flex flex-wrap" style="gap: 10px 40px;">
                        <div class="flex items-center gap-3">
                            ...
                            @include('dashboard.components.checkbox-button', [
                                ...
                                'id' => 'is_active',
                                'value' => '1',
                                'checked' => old('is_active', $marketingPackage->is_active),
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingFeatures = {!! json_encode($marketingPackage->features ?? []) !!};
            if (existingFeatures.length > 0) {
                existingFeatures.forEach((feature, index) => {
                    document.getElementById('addFeatureBtn').click();
                    const row = 'div[data-index="' + index + '"]';
                    document.querySelector(`input[name="feature_title_en[${index}]"]`).value = feature.title_en || '';
                    document.querySelector(`input[name="feature_title_ar[${index}]"]`).value = feature.title_ar || '';
                });
            }

            // Load old form data if validation failed
            const oldFeatures = {!! json_encode(old('feature_title_en', [])) !!};
            if (oldFeatures.length > 0) {
                // Clear existing features added
