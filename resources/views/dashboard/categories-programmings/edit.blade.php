...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('main.edit_categories_programming') }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb m-0 float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ __('main.edit') }}</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="{{ route('dashboard.categories-programmings.update', $image) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label>{{ __('main.current_image') }}</label>
                            <div class="mb-2">
                                <img src="{{ Storage::url($image->image) }}" alt="{{ $image->alt_text }}" style="max-width: 300px; height: auto;">
                            </div>
                        </div>

                        {{-- Image Upload --}}
                        <div class="form-group">
                            <label for="image">{{ __('main.change_image') }}</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('alt_text') is-invalid @enderror" id="alt_text" name="alt_text"
                                placeholder="{{ __('main.alt_text') }}" value="{{ old('alt_text', $image->alt_text) }}" maxlength="255">
                            @error('alt_text')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('order') is-invalid @enderror" id="order" name="order" placeholder="0"
                                value="{{ old('order', $image->order) }}" min="0">
                            @error('order')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
