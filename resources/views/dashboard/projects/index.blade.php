    ...
    <div class="w-full">
        ...
        <div class="flex flex-wrap gap-4 mb-6">
            <div class="flex-1 text-center p-4 bg-gray-50 shadow-lg radius-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-gray-800" id="stat-total">{{ count($projects) }}</div>
                <small class="text-primary font-semibold text-nowrap">{{ __('main.total_projects') }}</small>
            </div>
            <div class="flex-1 text-center p-4 bg-gray-50 shadow-lg radius-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-green-600" id="stat-active">{{ $projects->where('is_active', true)->count() }}</div>
                <small class="text-primary font-semibold text-nowrap">{{ __('main.active') }}</small>
            </div>
            <div class="flex-1 text-center p-4 bg-gray-50 shadow-lg radius-lg border border-gray-200 z--1">
                <div class="text-2xl font-bold text-red-600" id="stat-inactive">{{ $projects->where('is_active', false)->count() }}</div>
        ...
        <div class="bg-white shadow-lg radius-lg">
            <div class="flex justify-between items-center p-4 border-gray-200">
                ...
                <div class="flex justify-between items-center gap-4">
                    ...
                    <a href="{{ route('dashboard.projects.create') }}" class="kt-btn kt-btn-outline-primary" style="color: var(--text_color); background-color: var(--button_color);" toggle-button>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-gray-100 border-b-2 border-gray-300">
                            <th class="p-4 text-left text-sm font-semibold text-gray-700">{{ __('main.logo') }}</th>
                    ...
                    <tbody>
                        @forelse($projects as $project)
                            <tr id="row-{{ $project->id }}" class="border-b border-gray-200 hover:bg-gray-50 transition" data-active="{{ (int) $project->is_active }}">
                                <td title="{{ $project->translations[app()->getLocale()]['name'] ?? '' }}" class="p-4">
                                    <div class="relative w-fit">
                                        @if ($project->image && checkExistFile($project->image))
                                            <img src="{{ asset('storage/' . $project->image) }}" alt="{{ $project->translations[app()->getLocale()]['name'] ?? '' }}"
                                ...
                                <td class="p-4" title="{{ $project->website ?? '--' }}">
                                    @if ($project->website)
