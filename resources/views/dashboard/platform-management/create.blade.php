...
@section('title', __('main.create_type', ['type' => __('main.platform_management')]))
@section('page-title', '📱 ' . __('main.create_type', ['type' => __('main.platform_management')]))

@section('content')
    <div class="kt-card mb-4">
        <div class="kt-card-header flex items-center justify-between gap-4">
            <h3 class="kt-card-title">{{ __('main.create_type', ['type' => __('main.platform_management')]) }}</h3>

            <a href="{{ route('dashboard.platform-management.index') }}" class="kt-btn kt-btn-outline-primary" style="color: var(--text_color); background-color: var(--button_color);" toggle-button>
                {{ __('main.back_to_platform_management') }}
            </a>
        </div>

        <div class="kt-card-body p-4">
            <form method="POST" action="{{ route('dashboard.platform-management.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="grid gap-4">
                    ...
                    <!-- Save Button -->
                    @include('dashboard.components.save-submit', ['models' => 'dashboard.platform-management', 'model' => 'platform_management'])
                </div>
            </form>
        </div>
    </div>
