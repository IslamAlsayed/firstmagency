@if (isset($record) && $record && getActiveUser()->can("$models-update"))
    <div class="relative group inline-block">
        <button class="px-3 py-1 cursor-pointer text-nowrap bg-gray-200 hover:bg-gray-300 rounded-md text-xs font-semibold transition" title="Change {{ $fieldName ?? 'Status' }}">
            <i class="fas fa-exchange-alt"></i>
            {{-- <span class="hidden sm:inline">{{ __('main.change') }}</span> --}}
        </button>

        <!-- Dropdown Menu -->
        <div class="absolute right-0 mt-1 w-40 bg-white border border-gray-300 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
            <div class="py-1">
                @foreach ($availableOptions as $option)
                    <form method="POST" action="{{ route('dashboard.models.changeStatus', [$models, $modelClass, $record->id, $option]) }}" style="display:inline;">
                        @csrf
                        @method('PATCH')
