...
@section('content')
    <div class="kt-card mb-4">
        <div class="kt-card-header flex items-center justify-between gap-4">
            <h3 class="kt-card-title">{{ __('main.create_type', ['type' => __('main.programming_category')]) }}</h3>

            <a href="{{ route('dashboard.programming-categories.index') }}" class="kt-btn kt-btn-outline-primary" style="color: var(--text_color); background-color: var(--button_color);" toggle-button>
        ...
        <div class="kt-card-body p-4">
            <form method="POST" action="{{ route('dashboard.programming-categories.store') }}" enctype="multipart/form-data">
                ...
                <div class="grid gap-6">
                    ...
                    <!-- Save Button -->
                    @include('dashboard.components.save-submit', ['models' => 'dashboard.programming-categories', 'model' => 'programming_category'])
                </div>
            </form>
        </div>
    </div>
