...
@section('title', __('main.edit_type', ['type' => __('main.review')]))
    ...
    <div class="shadow-md radius-lg p-6">
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-4 pb-6">
            <div class="flex flex-col justify-center gap-2">
                <h1 class="text-xl font-medium leading-none text-mono">
                    {{ __('main.edit_type', ['type' => __('main.review')]) }}
                </h1>
                <div class="flex items-center gap-2 text-sm font-normal text-secondary-foreground">
                    🔄 {{ __('main.review') }}: <span class="font-semibold">{{ $review->name }}</span>
                </div>
            </div>
        ...
        <form action="{{ route('dashboard.reviews.update', $review) }}" method="POST" enctype="multipart/form-data" id="reviewForm">
            ...
            <div class="grid gap-4 lg:gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $review->name) }}"
                                    ...
                                    placeholder="{{ __('main.enter', ['field' => __('main.name')]) }}">
                                @error('name')
                            ...
                            <div>
                                ...
                                <select id="country" name="country"
                                    ...
                                    <option value="SA" {{ old('country', $review->country) == 'SA' ? 'selected' : '' }}>🇸🇦 Saudi Arabia</option>
                                    <option value="AE" {{ old('country', $review->country) == 'AE' ? 'selected' : '' }}>🇦🇪 United Arab Emirates</option>
                                    <option value="EG" {{ old('country', $review->country) == 'EG' ? 'selected' : '' }}>🇪🇬 Egypt</option>
                                    <option value="KW" {{ old('country', $review->country) == 'KW' ? 'selected' : '' }}>🇰🇼 Kuwait</option>
                                    <option value="QA" {{ old('country', $review->country) == 'QA' ? 'selected' : '' }}>🇶🇦 Qatar</option>
                                    <option value="BH" {{ old('country', $review->country) == 'BH' ? 'selected' : '' }}>🇧🇭 Bahrain</option>
                                    <option value="OM" {{ old('country', $review->country) == 'OM' ? 'selected' : '' }}>🇴🇲 Oman</option>
                                    <option value="JO" {{ old('country', $review->country) == 'JO' ? 'selected' : '' }}>🇯🇴 Jordan</option>
                                    <option value="LB" {{ old('country', $review->country) == 'LB' ? 'selected' : '' }}>🇱🇧 Lebanon</option>
                                    <option value="US" {{ old('country', $review->country) == 'US' ? 'selected' : '' }}>🇺🇸 United States</option>
                                </select>
                                @error('country')
                                    <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="rate" name="rate"
                                    ...
                                    <option value="5" {{ old('rate', $review->rate) == 5 ? 'selected' : '' }}>⭐⭐⭐⭐⭐ {{ __('main.excellent') }}</option>
                                    <option value="4" {{ old('rate', $review->rate) == 4 ? 'selected' : '' }}>⭐⭐⭐⭐ {{ __('main.good') }}</option>
                                    <option value="3" {{ old('rate', $review->rate) == 3 ? 'selected' : '' }}>⭐⭐⭐ {{ __('main.fair') }}</option>
                                    <option value="2" {{ old('rate', $review->rate) == 2 ? 'selected' : '' }}>⭐⭐ {{ __('main.poor') }}</option>
                                    <option value="1" {{ old('rate', $review->rate) == 1 ? 'selected' : '' }}>⭐ {{ __('main.very_poor') }}</option>
                                </select>
                                @error('rate')
                                    <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                                @enderror
                    ...
                    <div class="kt-card-body p-4">
                        ...
                        <div>
                            ...
                            <textarea id="comment" name="comment" rows="6"
                                ...
                                placeholder="{{ __('main.enter', ['field' => __('main.message')]) }}">{{ old('comment', $review->comment) }}</textarea>
                            @error('comment')
                                <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                            @enderror
                ...
                <div class="shadow-md radius-lg">
                    ...
                    <div class="kt-card-body p-4">
                        <div class="grid grid-cols-1 gap-6">
                            <!-- Photo Upload -->
                            @include('dashboard.components.photo', ['record' => $review])

                            <!-- Audio Display (Read-only for edit) -->
                            <div>
                                ...
                                @else
                                    ...
                                    <div id="audioUploadTab" class="audio-content">
                                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 transition"
                                            id="audioDropZone">
                ...
                <!-- Update Button -->
                @include('dashboard.components.update-submit', ['models' => 'dashboard.reviews', 'model' => 'review'])
            </div>
        </form>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const words = comment.value.trim().split(/\s+/).filter(w => w.length > 0).length;
            document.getElementById('wordCount').textContent = words;
        });
    </script>
@endpush
