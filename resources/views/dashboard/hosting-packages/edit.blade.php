...
@section('title', __('main.edit_type', ['type' => __('main.hosting_package')]))
@section('page-title', '📦 ' . __('main.edit_type', ['type' => __('main.hosting_package')]))

@section('content')
    <div class="shadow-lg radius-lg p-4">
        <form method="POST" action="{{ route('dashboard.hosting-packages.update', $hostingPackage->id) ?? '#' }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid gap-4 lg:gap-6">
                    ...
                    <div class="grid gap-4">
                        <div>
                            ...
                            <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500" id="title_ar" name="title_ar" required
                                value="{{ old('title_ar', $hostingPackage->translations['ar']['title']) }}" placeholder="أدخل العنوان بالعربية">
                            @error('title_ar')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select class="kt-select basic-single" id="category" name="category" required>
                            ...
                            <option value="hosting" {{ old('category', $hostingPackage->category) == 'hosting' ? 'selected' : '' }}>
                                {{ __('main.hosting_packages_shared') }}</option>
                            <option value="reseller" {{ old('category', $hostingPackage->category) == 'reseller' ? 'selected' : '' }}>
                                {{ __('main.hosting_packages_reseller') }}</option>
                            <option value="vps" {{ old('category', $hostingPackage->category) == 'vps' ? 'selected' : '' }}>
                                {{ __('main.hosting_packages_vps') }}</option>
                            <option value="servers" {{ old('category', $hostingPackage->category) == 'servers' ? 'selected' : '' }}>
                                {{ __('main.hosting_packages_dedicated') }}</option>
                        </select>
                        @error('category')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                ...
                <!-- Photo Upload -->
                @include('dashboard.components.photo', ['column' => 'image', 'record' => $hostingPackage])
                ...
                <div class="flex flex-wrap" style="gap: 10px 40px;">
                    <div class="flex items-center gap-3">
                        ...
                        @include('dashboard.components.checkbox-button', [
                            ...
                            'id' => 'is_active',
                            'value' => '1',
                            'checked' => old('is_active', $hostingPackage->is_active),
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @include('dashboard.components.checkbox-button', [
                            ...
                            'id' => 'is_popular',
                            'value' => '1',
                            'checked' => old('is_popular', $hostingPackage->is_popular),
                ...
                <!-- Update Submit -->
                @include('dashboard.components.update-submit', ['models' => 'dashboard.hosting-packages', 'model' => 'hosting_package'])
        </form>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingFeatures = @json($hostingPackage->features ?? []);
            existingFeatures.forEach((feature, index) => {
                addFeatureRow(index, feature);
            });

            // Add new empty feature
            let featureCount = existingFeatures.length;
            addFeatureBtn.addEventListener('click', function(e) {
                e.preventDefault();
                addFeatureRow(featureCount++);
            });

            function addFeatureRow(index, feature = {}) {
                const featureHTML = `
                    <div class="feature-row border border-gray-200 rounded-lg p-4 bg-gray-50" data-index="${index}">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="feature_label_en[${index}]" rows="2"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500">${feature.label_en || ''}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-600 mb-1">{{ __('main.label') }} AR</label>
                                <textarea name="feature_label_ar[${index}]" rows="2"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500">${feature.label_ar || ''}</textarea>
                            </div>
                        </div>
                    </div>
                `;
