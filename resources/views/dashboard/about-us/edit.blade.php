        ...
        <form action="{{ route('dashboard.about-us.update', $aboutUs->id) }}" method="POST" enctype="multipart/form-data" class="shadow-md radius-lg">
            ...
            <div class="kt-card-body">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" class="kt-input w-full @error('description') error @enderror" rows="5" placeholder="{{ __('main.description') }}">{{ old('description', $aboutUs->description) }}</textarea>
                    @error('description')
                        <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                @if ($aboutUs->image && checkExistFile($aboutUs->image))
                    <div class="mb-6">
                        <label class="block text-sm font-medium mb-2">{{ __('main.current_image') }}</label>
                        <img src="{{ asset('storage/' . $aboutUs->image) }}" alt="{{ $aboutUs->alt_text }}" class="w-32 h-32 object-cover rounded">
                    </div>
                @endif
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="w-4 h-4" {{ old('is_active', $aboutUs->is_active) ? 'checked' : '' }}>
